...
| **Codex CLI + Codex app** | `~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl` | Real (`turn_completed.usage` or `token_count` events), with estimation fallback. Emitted as `codex-cli` and `codex-app` rows |
...
On WSL, adapters also check Windows profiles from `USERPROFILE` and `/mnt/c/Users/*` where relevant. Codex also honours `CODEX_HOME`. If Kiro CLI has no structured `sessions/cli` data, the adapter falls back to `.kiro/.cli_bash_history` and reports prompt-only `human_input_tokens`, not billable model input.
...
| `input_tokens` | Billable model input tokens from provider usage or a context estimate. Human-only prompt records do not add to this field unless provider usage reports them as part of an API call |
| `output_tokens` | Tokens generated by the model (responses, tool calls) |
| `estimated_tokens` | Subset of billable input/output tokens estimated via tokenizer (not API-reported) |
...
| `human_input_tokens` | Tokenizer-estimated text typed by the user. Tracked separately so prompt text does not inflate API-equivalent input cost |
| `tool_calls` | Total number of tool invocations in the session |
| `requests` | Number of API-call-like records with billable tokens or tool activity |
...
Where a tool doesn't persist real token counts (Kiro CLI, Kiro IDE, legacy Copilot CLI) or writes placeholder values (some Claude Code assistant turns), the adapter estimates the API-call context with Anthropic's Claude tokenizer and flags billable input/output with `estimated: true`. User-authored text is also tokenized into `human_input_tokens`, but it is not counted as billable input by itself.
